...
function checkDynamicDisasters() {
    ...
    const checkAndToggle = (metricName, threshold, levelKey) => {
        ...
        const disasterNameKey = `${metricName}_${levelKey}`;
...
async function loadGameData() {
    try {
        ...
        gameData = await response.json(); // 리로드
        metrics = { ...gameData.initialMetrics };
        initGame();
    } catch (error) {
